...
# Definir colores para cada función
colors = ['blue', 'green', 'red', 'black']

# Nombres de las funciones
function_names = ['$|\Phi(x, t)|$', '$\mathcal{Re}\{\Phi(x, t)\}$', '$\mathcal{Im}\{\Phi(x, t)\}$', 'Potencial']
...
xs = frames_data[0][0]
lines = []
for j, ys in enumerate(frames_data[0][1:]):
    # Utiliza un color diferente para cada función
    line, = ax.plot(xs, ys, "-", color=colors[j], linewidth=1, label=function_names[j])
    lines.append(line)
...
def update(j_frame, frames_data, lines, functions_to_plot):
    xs = frames_data[j_frame][0]
    visible_functions = []  # Lista para almacenar las funciones visibles

    # Itera sobre las diferentes curvas
    for j_curve, ys in enumerate(frames_data[j_frame][1:]):
        # Actualiza la curva solo si está en la lista de funciones a trazar
        if j_curve + 1 in functions_to_plot:
            lines[j_curve].set_data(xs, ys)
            lines[j_curve].set_visible(True)  # Hace visible la línea
            visible_functions.append(function_names[j_curve])  # Agrega el nombre de la función visible
        else:
            lines[j_curve].set_data([], [])  # Si no está en la lista, oculta la línea
            lines[j_curve].set_visible(False)  # Hace invisible la línea

    # Actualiza la leyenda con las funciones visibles
    ax.legend([line for line in lines if line.get_visible()], visible_functions)
...
if nframes > 1:
    animation = FuncAnimation(
        fig, update,
        fargs=(frames_data, lines, functions_to_plot), frames=len(frames_data), blit=True, interval=interval)
